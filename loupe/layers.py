...
class ProbMask(Layer):
    ...
    def __init__(self, slope=10,
        ...
        self.slope = tf.Variable(slope, dtype=tf.float32, name='prob_slope')
...
class ResidualProbMask(Layer):
    ...
    def __init__(self, slope=10,
        ...
        if initializer == None:
            self.initializer = self._logit_random_uniform
        else:
            self.initializer = initializer

        if r_only:
            self.initializer = Constant(umask)

        # self.slope = tf.Variable(slope, dtype=tf.float32, name='prob_slope')
        self.umask = umask
        self.r_only = r_only
        super(ResidualProbMask, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.mult = self.add_weight(name='logit_weights',
                                    ...
                                    initializer=self.initializer,
                                    trainable=True)
        # self.wt1 = self.add_weight(name='wt1', initializer='ones', trainable=True)

        super(ResidualProbMask, self).build(input_shape)  # Be sure to call this somewhere!

    def call(self, x):
        t1_input = x
        logit_weights = 0 * t1_input[..., 0:1] + self.mult
        # return tf.sigmoid(self.slope * (logit_weights + self.umask[...,tf.newaxis] * self.wt1))
        # return tf.sigmoid(self.slope * (logit_weights + 1 * self.umask[...,tf.newaxis]))
        if self.r_only:
            res = tf.nn.relu(tf.minimum(logit_weights, 1))
        else:
            res = tf.nn.relu(tf.minimum(logit_weights, 1) + 1 * self.umask[...,tf.newaxis])
        return res
    ...
    def _logit_random_uniform(self, shape, dtype=None, eps=0.001):
        x = K.random_uniform(shape, dtype=dtype, minval=eps, maxval=0.1 - eps)  # [0, 1]
        return x
...
class ThresholdRandomMask(Layer):
    ...
    def __init__(self, slope = 12, **kwargs):
        ...
        if slope is not None:
            self.slope = tf.Variable(slope, dtype=tf.float32, name='threshold_slope')
    ...
    def call(self, x):
        ...
        if self.slope is not None:
            return tf.sigmoid(self.slope * (inputs-thresh)), tf.cast(inputs > thresh, tf.float32)
        else:
            return tf.cast(inputs > thresh, tf.float32)
